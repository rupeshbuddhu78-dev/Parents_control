    ...
    try {
        ...
        if (type === 'location') {
            ...
            finalData.sort((a, b) => (a.name || "").localeCompare(b.name || ""));
             ...
             const appMap = { 'com.whatsapp': 'whatsapp', 'com.whatsapp.w4b': 'whatsapp', 'com.instagram.android': 'instagram', 'com.snapchat.android': 'snapchat' };
             const resolveChatAppKey = (pkg) => {
                 if (!pkg) return null;
                 const p = String(pkg).toLowerCase();
                 if (appMap[pkg]) return appMap[pkg];
                 if (p.includes('whatsapp')) return 'whatsapp';
                 if (p.includes('instagram')) return 'instagram';
                 if (p.includes('snapchat')) return 'snapchat';
                 return null;
