...
const ChatMessageSchema = new mongoose.Schema({
    ...
    msgKey: { type: String, required: true } // dedupe key
...
async function saveChatMessages(deviceId, appKey, messages) {
    ...
    for (const m of messages) {
        try {
            ...
            if (!text || !conversation) continue;
            const direction = String(m.direction || 'IN').toUpperCase() === 'OUT' ? 'OUT' : 'IN';
            const ts = Number(m.timestamp) || Date.now();
            // Dedupe: device+app+conv+dir+text within ~same second bucket
            const msgKey = [id, app, conversation.toLowerCase(), direction, text.slice(0, 200), Math.floor(ts / 5000)].join('|');
            ...
            if (!(e && e.code === 11000)) console.error('saveChatMessages', e.message);
        }
    }
    return saved;
}
